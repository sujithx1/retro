  ...
  try {
    ...
    if (!orderDetails) {
      ...
      if (status == 'pending' || 'completed') {
        ...
        if (orderData.products[productIndex].product_orderStatus === 'cancelled') {
          res.status(400).json({
            success: false,
            message: 'sorry your order Already cancelled ',
          });
        } else if (orderData.products[productIndex].product_orderStatus === 'pending') {
          orderData.products[productIndex].product_orderStatus = status;
          await orderData.save();
          res.status(200).json({ success: true, message: `Oder ${status}...` });
        } else if (orderData.products[productIndex].product_orderStatus === 'completed') {
          orderData.products[productIndex].product_orderStatus = status;
          await orderData.save();
          ...
          if (Array.isArray(order.products)) {
            ...
            if (orderDetails) {
              ...
              res.render('admin/orderDetail', {
                order: orderDetails,
                userOrder: orderData.length > 0 ? orderData[0].address : null,
...
const orderDetailsUpdateStatus = async (req, res) => {
  try {
    ...
    const orderData = await Order.findById(orderIdd)
      .populate('products.productId')
      .populate('products.coupon');
    if (orderData) {
