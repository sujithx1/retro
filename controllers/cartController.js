...
const loadShopCart = async (req, res) => {
  try {
    ...
    const userData = await Cart.findOne({ userId: userId }).populate({
  ...
  try {
    ...
    if (totalStock < quantity) {
      ...
      if (!cart) {
        ...
        const existingProductIndex = cart.products.findIndex((pro) =>
          pro.productId.equals(productId)
        );
    ...
    if (userData) {
      ...
      if (!product) {
        ...
        if (qty > availableStock) {
          res.status(400).json({
            success: false,
            error: `${product.productId.name} (${product.size}) has only ${availableStock} product(s) available.`,
          });
          ...
          totalstock =
            product.productId.stock.XS +
            product.productId.stock.S +
            product.productId.stock.M +
            product.productId.stock.L +
            product.productId.stock.XL +
            product.productId.stock.XXL;
          ...
          if (qty > totalstock) {
            ...
            return res
              .status(400)
              .json({ error: `Out of Stock. This product has only ${totalstock} available.` });
          } else {
            ...
            else res.status(500).json({ success: false, error: 'Failed to update quantity' });
          }
        }
      }
    }
