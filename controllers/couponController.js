...
const addCoupons = async (req, res) => {
  try {
    ...
    const uniqueCoupon = await Coupon.findOne({
      coupon_Code: { $regex: couponCode, $options: 'i' },
    });
    if (uniqueCoupon) {
...
const ApplayingCoupon = async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ success: true, message: 'Coupon applied successfully!', couponId: couponId });
  } catch (error) {
    console.error('Error from coupon controller ApplayingCoupon', error);
    res
      .status(500)
      .json({ success: false, message: 'An error occurred while applying the coupon.' });
  }
};
