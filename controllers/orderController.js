...
const loadOder = async (req, res) => {
  try {
    ...
    if (cartData) {
      ...
      const unusedCoupons = allCoupons.filter(
        (coupon) => !user.coupons.some((usedCoupon) => usedCoupon._id.equals(coupon._id))
      );
      ...
      if (!OrderData) {
        ...
        for (const item of cartData.products) {
          ...
          if (updatedProduct) {
            await updatedProduct.save();
            console.log('Updated product:', updatedProduct);
          } else console.log('error making uipdate the product');
        }
      }
...
const walllet_payment = async (req, res) => {
  try {
    ...
    if (amount > OrderData.Wallet) {
      res
        .status(200)
        .json({ success: false, message: `You Have Only ${OrderData.Wallet} in your Wallet` });
...
const payment_failure = async (req, res) => {
  try {
    ...
    if (saving) res.status(200).json({ success: true, cartId: cartId });
