...
const insertUser = async (req, res) => {
  ...
  try {
    ...
    if (existEmail && existMobile) {
      ...
      res.render('users/signup', { msg: 'Email is Already Exist' });
      console.log('Email is Already Exist');
    } else if (existMobile) {
      res.render('users/signup', { msg: 'Mobile Number is Alredy Exist' });
      ...
      if (entrOtp === sessionOtp && Date.now() < expOtp) {
        ...
        if (referalData) {
          ...
          if (referralUpdate) {
            ...
            if (orderData) console.log('saved');
        ...
        res.render('users/verification', { msg: 'Incorrect OTP or Expired OTP. Please try again' });
      }
    }
  } catch (error) {
...
const loadproductDetails = async (req, res) => {
  try {
    ...
    if (productDetails) {
      ...
      totalstock =
        productDetails.stock.XS +
        productDetails.stock.S +
        productDetails.stock.M +
        productDetails.stock.L +
        productDetails.stock.XL +
        productDetails.stock.XXL;
...
const editAddress = async (req, res) => {
  try {
    ...
    if (findAddress) {
      ...
      if (saved) res.redirect('/profile#address');
      else res.render('users/address_edit', { msg: 'address not edited' });
    }
  } catch (error) {
...
const setDefaultAddress = async (req, res) => {
  try {
    ...
    if (userAddress) {
      ...
      const findAddress = await userAddress.address.find(
        (addr) => addr._id.toString() === addressId
      );
      if (findAddress) {
  ...
  try {
    ...
    if (!orderDetails) {
      ...
      if (orderDetails.payment_status === 'Success') {
        ...
        const addWallet = await Order.findByIdAndUpdate(
          orderId,
          { $inc: { Wallet: wallet } },
          { new: true }
        );
        ...
        } else console.log('wallet not added');
      }

      await orderData.save();

      const responseMessage = walletMessage
        ? `Order cancelled successfully. ${walletMessage}`
...
const sumbmitReview = async (req, res) => {
  try {
    ...
    if (savedRating) res.status(201).json({ success: true, savedRating });
